...
const Appointments = ({ appointments }) => {
  return (
    <div className="flex-1 mx-2 my-2 bg-slate-950 rounded-md p-4">
      {/* Header Row */}
      <div className="w-full grid grid-cols-[20%_20%_20%_10%_20%] border-b border-gray-700 text-gray-400 text-[15px] font-semibold py-3">
      ...
      {appointments.length > 0 ? (
        appointments.map(app => (
          <div
            key={app._id}
            className="w-full grid grid-cols-[20%_20%_20%_10%_20%] border-b border-gray-800 py-4 text-sm text-gray-300 hover:bg-slate-800 transition-all duration-300"
          >
            ...
            <div className="px-4 text-left font-medium">{app.type}</div>
          </div>
        ))
      ) : (
