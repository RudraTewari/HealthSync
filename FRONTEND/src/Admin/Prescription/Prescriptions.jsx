...
const Prescriptions = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/prescriptions") // Change URL to your backend
      .then((res) => res.json())
          ...
          <div className="col-span-10 bg-slate-900 h-full flex flex-col">
            ...
            <div className="flex-1 mx-2 my-2 bg-slate-950 rounded-md p-4">
              <div className="flex flex-col gap-2 ml-3">
                <h1 className="text-[30px] font-bold text-amber-100 ">
                  All Prescriptions
                </h1>
              ...
              {/* Table Header */}
              <div className="w-full grid grid-cols-[20%_15%_10%_10%_25%] justify-evenly border-b font-semibold border-gray-700 text-gray-400 text-[15px] py-4">
                <div>Patient</div>
                <div>Doctor</div>
                <div>Date</div>
                <div>Status</div>
              ...
              {prescriptions.length > 0 ? (
                prescriptions.map((pres, index) => (
                  <div
                    key={index}
                    className="w-full grid grid-cols-[20%_15%_10%_10%_25%] justify-evenly border-b border-gray-800 hover:bg-slate-600 hover:text-slate-300 transition-all duration-300 py-5 text-sm text-gray-300"
                  >
